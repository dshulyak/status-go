...
func NewManager(nodeManager common.NodeManager, accountManager common.AccountManager) *Manager {
	return &Manager{
		...
		ethTxClient:    NewEthTxClient(nodeManager.RPCClient()),
	}
}
...
func (m *Manager) completeTransaction(selectedAccount *common.SelectedExtKey, queuedTx *common.QueuedTx, password string) (gethcommon.Hash, error) {
	...
	if args.GasPrice == nil {
		...
		defer cancel2()
		gasPrice, err = m.ethTxClient.SuggestGasPrice(ctx2)
		if err != nil {
			return emptyHash, err
		}
	...
	ctx3, cancel3 := context.WithTimeout(context.Background(), cancelTimeout)
	defer cancel3()
	gas, err := m.ethTxClient.EstimateGas(ctx3, ethereum.CallMsg{
		From:     args.From,
		To:       args.To,
		GasPrice: gasPrice,
		Value:    value,
	...
	ctx4, cancel4 := context.WithTimeout(context.Background(), cancelTimeout)
	defer cancel4()
	if err := m.ethTxClient.SendTransaction(ctx4, tx); err != nil {
		return emptyHash, err
	}
