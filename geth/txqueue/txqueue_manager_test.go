...
func (s *TxQueueTestSuite) SetupTest() {
	...
	s.server, s.ethService = fake.NewTestServer()
...
func (s *TxQueueTestSuite) TestCompleteTransaction() {
	...
	s.ethService.On("GetTransactionCount", mock.Anything, mock.Anything, mock.Anything).Return(&nonce, nil)
	s.ethService.On("GasPrice", mock.Anything).Return(big.NewInt(10), nil)
	gas := hexutil.Big(*big.NewInt(defaultGas + 1))
	s.ethService.On("EstimateGas", mock.Anything, mock.Anything).Return(&gas, nil)
	s.ethService.On("SendRawTransaction", mock.Anything, mock.Anything).Return(gethcommon.Hash{}, nil)
...
func (s *TxQueueTestSuite) TestInvalidPassword() {
	...
	gas := hexutil.Big(*big.NewInt(defaultGas + 1))
	s.ethService.On("EstimateGas", mock.Anything, mock.Anything).Return(&gas, nil)
	s.ethService.On("SendRawTransaction", mock.Anything, mock.Anything).Return(gethcommon.Hash{}, keystore.ErrDecrypt)
